    ...
    <div style={containerStyle}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '30px', marginBottom: '30px' }}>
        <div>
          ...
          <p style={{ color: '#a1a1aa', fontSize: '1.2rem', marginBottom: '5px' }}><strong>Email:</strong> {user.email}</p>
          <p style={{ color: '#a1a1aa', fontSize: '1.2rem', marginBottom: '15px' }}><strong>Joined:</strong> {formatDate(user.createdAt)}</p>
        ...
        <div style={{ display: 'grid', gap: '20px' }}>
          {orders.map(order => (
            <div key={order._id} style={{ background: '#09090b', padding: '20px', borderRadius: '12px', border: '1px solid #27272a', display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: '20px' }}>
              ...
              <div>
                <span style={{ 
                  background: (order.status || 'pending').toLowerCase() === 'delivered' ? 'rgba(16,185,129,0.1)' : (order.status || 'pending').toLowerCase() === 'shipped' ? 'rgba(59,130,246,0.1)' : 'rgba(245,158,11,0.1)', 
                  color: (order.status || 'pending').toLowerCase() === 'delivered' ? '#10b981' : (order.status || 'pending').toLowerCase() === 'shipped' ? '#3b82f6' : '#f59e0b',
